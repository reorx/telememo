...
These tests hit the real Telegram API using credentials from
``~/.config/telememo/config.py``. They only run when
``ALLOW_LOGIN=1`` is set, so a normal ``pytest`` run never logs into Telegram
by accident.
"""
...
pytestmark = pytest.mark.skipif(
    os.environ.get('ALLOW_LOGIN') != '1',
    reason='integration tests hit real Telegram; set ALLOW_LOGIN=1 to run',
)
