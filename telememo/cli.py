...
@click.group()
@click.option('--channel-name', '-c', help='Channel username (e.g., @channelname or channelname)')
...
def sync(ctx, skip_comments: bool, limit: int, full: bool):
    ...
    async def run_sync():
        ...
        click.echo(
            f'  Messages: {result.messages_added} added, {result.messages_updated} updated, {result.messages_unchanged} unchanged'
        )
        if not skip_comments:
            click.echo(
                f'  Comments: {result.comments_added} added, {result.comments_updated} updated, {result.comments_unchanged} unchanged'
            )
        click.echo('=' * 50)
...
@click.option('--comments', is_flag=True, help='Search in comments only')
@click.option(
    '--include-comments',
    is_flag=True,
...
@click.pass_context
def search(ctx, query: str, limit: int, comments: bool, include_comments: bool):
    ...
    Examples:
      ...
      telememo search "keyword"                       # Search in messages (default)
      telememo search "keyword" --comments            # Search in comments only
